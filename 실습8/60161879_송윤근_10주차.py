from tensorflow.keras import Model
from tensorflow import keras
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import Dense, Dropout, Flatten
from tensorflow.keras.layers import Conv2D, MaxPooling2D
from tensorflow.keras.datasets import mnist
import matplotlib.pyplot as plt
import numpy as np
#필요한 모듈을 불러옵니다.
def load_data():
    # 먼저 MNIST 데이터셋을 로드하겠습니다. 케라스는 `keras.datasets`에 널리 사용하는 데이터셋을 로드하기 위한 함수를 제공합니다. 이 데이터셋은 이미 훈련 세트와 테스트 세트로 나누어져 있습니다. 훈련 세트를 더 나누어 검증 세트를 만드는 것이 좋습니다:

    (X_train_full, y_train_full), (X_test, y_test) = mnist.load_data()
    #mnist의 데이터를 불러와 변수에 저장을 합니다, 테스트와 트레이닝 부분이 나눠집니다.

    X_train_full = X_train_full.astype(np.float32)
    #데이터를 float타입으로 변경합니다
    X_test = X_test.astype(np.float32)
    #데이터를 float타입으로 변경합니다
    #print(X_train_full.shape, y_train_full.shape)
    #print(X_test.shape, y_test.shape)
    return X_train_full, y_train_full, X_test, y_test
    #값을 넘겨줍니다.
def data_normalization(X_train_full, X_test):
    # 전체 훈련 세트를 검증 세트와 (조금 더 작은) 훈련 세트로 나누어 보죠. 또한 픽셀 강도를 255로 나누어 0~1 범위의 실수로 바꾸겠습니다.

    X_train_full = X_train_full / 255\

    X_test = X_test / 255.
    # x값이 그림 데이터로 크기를 줄이기 위해 255로 나눕니다.
    train_feature = np.expand_dims(X_train_full, axis=3)
    test_feature = np.expand_dims(X_test, axis=3)
    #합성곱을 하기 위해서는 채널값이 필요합니다. 즉 3개의 파라미터값이 필요하기 때문에 디멘션을 늘려주는 것입니다.
    print(train_feature.shape, train_feature.shape)
    print(test_feature.shape, test_feature.shape)
    #바꾼 후 shape를 확인하기 위해 출력합니다.
    return train_feature,  test_feature
    #값을 리턴해줍니다.

def draw_digit(num):
    for i in num:
        for j in i:
            if j == 0:
                print('0', end='')
            else :
                print('1', end='')
        print()


def makemodel(X_train, y_train, X_valid, y_valid, weight_init):
    model = Sequential()
    model.add(Conv2D(32, kernel_size=(3, 3),  activation='relu'))
    #conv2d를 추가하는데 커널사이즈는 3*3이고 활성화함수는 relu입니다. 커널의 개수는 32개입니다.
    model.add(MaxPooling2D(pool_size=2))
    #poolingsize를 2로 해 maxpooling을 만들어줍니다
    model.add(Conv2D(64, kernel_size=(3, 3), activation='relu'))
    # conv2d를 추가하는데 커널사이즈는 3*3이고 활성화함수는 relu입니다. 커널의 개수는 64개입니다.
    model.add(MaxPooling2D(pool_size=2))
    # poolingsize를 2로 해 maxpooling을 만들어줍니다
    model.add(Dropout(0.25))
    #과적합방지를 위해 드랍아웃을 합니다.
    model.add(Flatten())
    #Flatten을 해줍니다.
    model.add(Dense(128, activation='relu'))
    #활성화함수가 relu인 은닉층을 128개 만듭니다.
    model.add(Dense(10, activation='softmax'))
    #softmax로 10개의 출력을 만듭니다.
    model.compile(loss='sparse_categorical_crossentropy',
                  optimizer='adam',
                  metrics=['accuracy'])
    #컴파일을 하는 부분입니다.


    return model
    #모델을 리턴해줍니다.
def plot_history(histories, key='accuracy'):
    plt.figure(figsize=(16,10))
    #fig를 생성해줍니다.
    for name, history in histories:
        val = plt.plot(history.epoch, history.history['val_'+key],
                       '--', label=name.title()+' Val')
        plt.plot(history.epoch, history.history[key], color=val[0].get_color(),
                 label=name.title()+' Train')
    #그림에 plot해줍니다. 각각이름을 설정해주고 val은 접선으로 표시합니다.
    plt.xlabel('Epochs')
    plt.ylabel(key.replace('_',' ').title())
    #x축과 y축의 이름을 설정해줍니다.
    plt.legend()
    #자동으로 감지하여 레전드를 작성
    plt.xlim([0,max(history.epoch)])
    #x값의 범위를 설정해준다
    plt.show()
import random


def draw_prediction(pred, k,X_test,y_test,yhat):
    samples = random.choices(population=pred, k=16)

    count = 0
    nrows = ncols = 4
    plt.figure(figsize=(12,8))

    for n in samples:
        count += 1
        plt.subplot(nrows, ncols, count)
        plt.imshow(X_test[n].reshape(28, 28), cmap='Greys', interpolation='nearest')
        tmp = "Label:" + str(y_test[n]) + ", Prediction:" + str(yhat[n])
        plt.title(tmp)
    #그림에 대해서 라벨과 예측값을 타이틀로 보여주는 함수입니다.
    plt.tight_layout()
    plt.show()

def evalmodel(X_test,y_test,model):
    yhat = model.predict(X_test)
    #x테스트값에 대한 추측값이 저장됩니다.
    yhat = yhat.argmax(axis=1)
    #확률값을 가지고 있기 때문에 가장 큰 값을 yhat에 저장합니다.
    print(yhat.shape)

    #이후는 정확하게 예측한지 판단
    answer_list = []

    for n in range(0, len(y_test)):
        if yhat[n] == y_test[n]:
            answer_list.append(n)
    #정확하게 예측한 값을 찾는 것이다.
    draw_prediction(answer_list, 16,X_test,y_test,yhat)
    #이 부분은 정확하게 예측한 값을 16개만 보여달라는 함수입니다..

    #이후는 정확하게 예측하지 못한 값입니다.
    answer_list = []

    for n in range(0, len(y_test)):
        if yhat[n] != y_test[n]:
            answer_list.append(n)

    draw_prediction(answer_list, 16,X_test,y_test,yhat)
    #이부분은 정확하게 예측하지 못한 값을 16개만 보여달라는 함수입니다.
def main():
    X_train, y_train, X_test, y_test = load_data()
    #load_data함수로 데이터를 불러옵니다.
    X_train, X_test = data_normalization(X_train,  X_test)
    #불러온 데이터를 normalization함수로 알맞게 바꿔줍니다.
    #show_oneimg(X_train)
    #show_40images(X_train, y_train)

    model= makemodel(X_train, y_train, X_test, y_test,'glorot_uniform')
    #모델을 만드는 함수를 불러와 모델을 만듭니다.


    baseline_history = model.fit(X_train,
                                 y_train,
                                 epochs=2,
                                 batch_size=512,
                                 validation_data=(X_test, y_test),
                                 verbose=2)
    #모델을 채워줍니다. x와 y의 트레인부분을 넣어주고 반복을 2를 줍니다. 베치사이즈는 512로 나눠서 합니다.
    #validation은 테스트값으로 할 예정이고 중간 결과값을 보여준다는 뜻으로 verbose를 2로 주었다.

    evalmodel(X_test, y_test, model)
    #evalmodel이라는 함수를 불러옵니다.
    plot_history([('baseline', baseline_history)])
    #그림을 그리는 함수를 불러옵니다.
main()
